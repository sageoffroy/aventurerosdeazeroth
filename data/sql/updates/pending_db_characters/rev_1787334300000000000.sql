...
-- SpellDraft: metaprogresión de cuenta + elecciones de talento por vida.
--
-- Las tablas account_* sobreviven a la muerte/eliminación del superviviente.
-- Las tablas character_* pertenecen solamente a la vida actual.

CREATE TABLE IF NOT EXISTS `spelldraft_account_progress` (
...
CREATE TABLE IF NOT EXISTS `spelldraft_account_talents` (
    `account_id` INT UNSIGNED NOT NULL,
    `talent_id` VARCHAR(64) NOT NULL,
    `unlocked_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`account_id`, `talent_id`),
    KEY `idx_spelldraft_account_talents_account` (`account_id`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `spelldraft_character_talents` (
    ...
    UNIQUE KEY `uq_spelldraft_character_talent_index`
        (`player_guid`, `talent_index`),
    KEY `idx_spelldraft_character_talents_guid` (`player_guid`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `spelldraft_pending_talent_offer` (
    `player_guid` INT UNSIGNED NOT NULL,
