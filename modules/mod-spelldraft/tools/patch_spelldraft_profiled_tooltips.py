...
from repair_profiled_tooltip_display import LUA as PROFILE_TOOLTIP_LUA
...
def patch_legacy_custom_alias_namespace(addon_dir: Path) -> bool:
    path = addon_dir / "CustomSpellAliases.lua"
    if not path.is_file():
        return False

    original = path.read_text(encoding="utf-8")
    namespace = "SpellDraft = SpellDraft or {}"

    if namespace in original:
        return False

    alias_marker = "SpellDraftCustomSpellAliases = {"
    if alias_marker not in original:
        raise TooltipError(
            f"{path}: legacy custom alias table marker not found"
        )
...
def main() -> None:
    ...
    try:
        ...
        if patch_legacy_custom_alias_namespace(addon_dir):
            changed = True
