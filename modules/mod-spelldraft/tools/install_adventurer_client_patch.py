#!/usr/bin/env python3
"""Install generated Aventureros de Azeroth MPQs into a WoW 3.3.5a client safely.

Client patch ownership is fixed by project convention:

* Z is the single official Aventureros de Azeroth patch family.
* Adventurer, SpellDraft, custom DBCs and future client changes all live in Z.
...
def verify_existing_owned_file(target: Path, expected_hash: str | None) -> None:
    ...
    if not expected_hash:
        raise SystemExit(
            "Reserved Aventureros patch slot is already occupied and there is no "
            f"project ownership record:\n  {target}\nNothing was overwritten."
        )
    actual = sha256(target)
    if actual != expected_hash:
        raise SystemExit(
            "Reserved Aventureros patch changed outside the project. Refusing to overwrite:\n"
            f"  {target}\n"
...
def main() -> None:
    ...
    if not source_root.is_file() or not source_locale.is_file():
        raise SystemExit(
            "Generated Aventureros patch pair not found. Expected:\n"
            f"  {source_root}\n"
            f"  {source_locale}"
        )
    ...
    target_locale = (
        client / "Data" / args.locale / f"patch-{args.locale}-{PROJECT_SUFFIX.lower()}.mpq"
    )
    ...
    print("Aventureros de Azeroth client patch installed in the official Z family:")
    print(f"  {target_root}")
    print(f"  {target_locale}")
    print(f"  locale: {args.locale}")
    print("  Z contains Adventurer + SpellDraft + project DBC/client changes")
